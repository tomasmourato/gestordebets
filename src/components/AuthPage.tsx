...
export default function AuthPage({ onAuthenticated }: AuthPageProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans text-slate-900 antialiased selection:bg-indigo-600 selection:text-white border-t-4 border-indigo-600">
      <div className="w-full max-w-sm bg-white border border-slate-200 rounded-2xl shadow-sm p-8">
        ...
        {/* Tabs */}
        <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1 mb-6" role="tablist">
          <button
            type="button"
            role="tab"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <AnimatePresence initial={false}>
            {mode === "signup" && (
              ...
              >
                <span className="block text-xs font-semibold text-slate-500 mb-1.5">Username</span>
                <input
                  type="text"
                  value={username}
                  ...
                  required
                  minLength={3}
                  className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-600/15 focus:border-indigo-600 transition-colors"
                />
              </motion.label>
            )}
          </AnimatePresence>
          ...
          <label className="block">
            ...
            />
            {mode === "signup" && (
              <span className="block text-[11px] text-slate-400 mt-1.5">Mínimo 8 caracteres.</span>
            )}
          </label>

          {error && (
            <div role="alert" className="bg-rose-50 border border-rose-200 text-rose-600 text-xs font-medium rounded-lg px-3 py-2.5">
              {error}
            </div>
          )}
        ...
        <p className="text-center text-xs text-slate-500 mt-5">
          {mode === "login" ? (
            <>
              ...
              <button type="button" onClick={() => switchMode("signup")} className="text-indigo-600 font-semibold hover:text-indigo-700">
                Regista-te
              </button>
            </>
          ) : (
            <>
              Já tens conta?{" "}
              <button type="button" onClick={() => switchMode("login")} className="text-indigo-600 font-semibold hover:text-indigo-700">
                Inicia sessão
              </button>
            </>
          )}
